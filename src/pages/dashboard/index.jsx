...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchDashboard() {
      try {
        ...
        // Inject color + id to each bank
        const processedBanks = (data.banks || []).map((b, i) => ({
          id: `BANK-${i + 1}`,
        ...
        setSelectedBankId(processedBanks[0]?.id || null);
      } catch (err) {
  ...
  const totalBranches = banks.reduce((acc, b) => acc + (b.totalBranches || 0), 0);
  const totalAgents = banks.reduce((acc, b) => acc + (b.totalAgents || 0), 0);
  const agentsPerBank = banks.map((b) => ({ name: b.name, agents: b.totalAgents || 0 }));
